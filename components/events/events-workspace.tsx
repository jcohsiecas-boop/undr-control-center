...
type EventItem = { id: string; eventId: string; slug: string; name: string; date: string; budget: string | number; status: "PLANNING" | "ACTIVE" | "CLOSED" | "CANCELLED"; lineItems: Line[] };
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-card/75 p-5">
        ...
        <form className="grid gap-3 lg:grid-cols-6" onSubmit={createEvent}>
          ...
          <Select value={eventDraft.status} onChange={(e) => setEventDraft({ ...eventDraft, status: e.target.value })}><option value="PLANNING">Planeacion</option><option value="ACTIVE">Activo</option><option value="CLOSED">Cerrado</option><option value="CANCELLED">Cancelado</option></Select>
          ...
          return (
            <Card key={event.id} className="bg-card/75 p-5 transition hover:-translate-y-0.5 hover:bg-card">
              <div className="flex items-start justify-between gap-3">
                ...
                <Badge tone={event.status === "CANCELLED" ? "red" : event.status === "CLOSED" ? "green" : "amber"}>{statusLabel[event.status]}</Badge>
              </div>
