  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <LineTable title="Ingresos" lines={incomeLines} actionLabel="Por cobrar" doneTone="green" projected={projectedIncome} actual={actualIncome} updateLine={updateLine} deleteLine={deleteLine} />
        <LineTable title="Egresos" lines={expenseLines} actionLabel="Por pagar" doneTone="red" projected={projectedExpense} actual={actualExpense} updateLine={updateLine} deleteLine={deleteLine} />
      </section>
    </div>
  );
}
...
function LineTable({ title, lines, actionLabel, doneTone, projected, actual, updateLine, deleteLine }: { title: string; lines: Line[]; actionLabel: string; doneTone: "green" | "red"; projected: number; actual: number; updateLine: (id: string, patch: Partial<Line> & { actioned?: boolean }) => void; deleteLine: (id: string) => void }) {
  return (
    <Card className="overflow-hidden bg-card/75">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-sm">
          ...
          <tbody>{lines.map((line) => <tr key={line.id} className="border-t border-border"><td className="p-3">{line.concept || "-"}</td><td>{line.quantity}</td><td>{money(Number(line.unitCost))}</td><td>{money(Number(line.projected))}</td><td><Input className="w-28" type="number" value={Number(line.actual)} onChange={(e) => updateLine(line.id, { actual: e.target.value })} /></td><td>{line.responsible ?? "-"}</td><td>{line.actionedAt ? <Badge tone={doneTone}><CheckCircle2 className="mr-1 h-3 w-3" /> En finanzas</Badge> : <Button size="sm" variant="secondary" onClick={() => updateLine(line.id, { actioned: true })}>{actionLabel}</Button>}<p className="mt-1 text-[10px] text-muted-foreground">{line.actionedAt ? `${line.actionedBy?.name ?? line.actionedBy?.email ?? "Usuario"} · ${new Date(line.actionedAt).toLocaleString("es-CO")}` : ""}</p></td><td><Button size="sm" variant="outline" onClick={() => deleteLine(line.id)}><Trash2 className="h-4 w-4" /></Button></td></tr>)}</tbody>
        </table>
      </div>
    </Card>
  );
